...
class PatternFinding(object):
    def __init__(self, contours_group, image):
        ...
        if not contours_group:
            ...
            contours, hierarchy = contours_group
    ...
    def CheckContourWithinContourHavingLevel(self, nooflevels):
        ...
        for index in range(len(self.Contours)):
            if self.IsPossibleQRContour(index, nooflevels) is True:
                patterns.append(self.Contours[index])
                patterns_indices.append(index)
        return patterns, patterns_indices

    def FindingQRPatterns(self, noof_levels):
        """This function filters to have only three QR patterns"""
        patterns, patterns_dictionary = self.CheckContourWithinContourHavingLevel(
            noof_levels)  # returns contours and list of indices
        # of the returned contours
        QRPatterns = []

        while len(patterns) < 3:
            noof_levels = noof_levels - 1
            if noof_levels <= 1:
                return QRPatterns
            patterns, patterns_dictionary = \
                self.CheckContourWithinContourHavingLevel(noof_levels)
    ...
    def CheckingRatioOfContours(self, index):
        ...
        if areaofsecondchild <= 0:
            return False
        return (areaoffirst / areaofsecondchild) > 1 and ((areaoffirst / areaofsecondchild) < 10)
